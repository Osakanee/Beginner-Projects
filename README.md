<div align="center">
<h1> Beginner projects (Python) </h1>
All of the projects can be used for other ideas and are just for practice and learning for future experience
<br>
Tiny scripts that are not-so-big are not on a folder.
<br><br>
</div>
<h2> Codetons </h2>
A tiny and simple turn based battle where you can choose one of the 3 codetons to battle Tiny Vin. <br>
Be advised that v1 version of codetons needs the module progress.
<br><br>
Requirements - pip:
<pre>
  pip install rich
</pre>
<h3> Projectmaker </h3>
A script that makes a project folder with assets and src subfolders for you<br>
Requirements:
<pre>pip install requirements.txt</pre>

